    ...
    >
      <div 
        className={`
          ...
          rounded-lg
          border-4 
          border-[#222] shadow-[0_25px_15px_-10px_rgba(0,0,0,0.8),0_10px_0px_#000]
          cursor-pointer
          overflow-hidden
          group
      ...
      >
        ...
        {/* Header Section */}
        <div className={`relative bg-[#222] text-[#e0e0e0] border-b-4 border-[#111] shadow-[0_4px_4px_rgba(0,0,0,0.5)] z-10 shrink-0 ${isExpanded ? 'h-auto flex items-stretch' : 'h-[80px] flex items-stretch'}`}>
          
          {useSplitLayout ? (
            <>
               <div className="w-4 bg-retro-gray border-r border-[#444] relative overflow-hidden shrink-0">
               ...
               <div className="flex-1 p-3 flex flex-col gap-2 min-w-0">
                  <div className="flex justify-between items-center border-b border-[#333] border-dashed pb-2">
                     <div className="flex items-center gap-3">
                        <div className="flex flex-col">
                           <div className="text-[10px] text-[#888] tracking-widest uppercase leading-none">Index.Date</div>
                  ...
                  <div className="w-full">
                     <div className="text-[10px] text-[#888] tracking-widest uppercase mb-0.5">Subject</div>
                     <h3 className="text-xl font-bold text-white whitespace-pre-wrap break-words leading-tight">
            ...
            <>
              ...
              <div className={`flex-1 px-4 flex justify-between relative overflow-hidden ${isExpanded ? 'py-4 items-center' : 'py-2 items-center'}`}>
                 <div className="flex flex-col shrink-0">
                    <div className="text-[10px] text-[#888] tracking-widest uppercase">Index.Date</div>
                    <div className="text-2xl font-bold tracking-tighter text-retro-amber text-shadow-amber font-mono">
                      {year}.{dateString}
                    </div>
                 </div>

                 <div className={`w-px bg-[#444] mx-4 shrink-0 ${isExpanded ? 'self-stretch my-1' : 'h-8'}`} />

                 <div className="flex-1 flex flex-col min-w-0">
                    <div className="text-[10px] text-[#888] tracking-widest uppercase">Subject</div>
                    <h3 className={`text-lg font-bold text-white group-hover:text-retro-orange transition-colors ${isExpanded ? 'whitespace-pre-wrap break-words leading-tight' : 'truncate'}`}>
                      {note.isProcessing ? 'PROCESSING...' : (note.analysis?.summary || 'NO_DATA')}
                    </h3>
                 </div>
        ...
        {/* Cassette Body */}
        <div className={`relative bg-[#d0d0d0] p-4 flex-1 flex flex-col gap-4`}>
           <div 
             className={`
                relative bg-[#1a1a1a] border-2 border-[#555] rounded-sm p-4 overflow-hidden shadow-[inset_0_4px_8px_rgba(0,0,0,0.8)] flex flex-col
           ...
           >
              ...
              <div className={`relative z-10 flex flex-col ${isExpanded ? 'h-auto' : 'h-full'}`}>
                <div className="flex items-center gap-2 mb-2 pb-2 border-b border-[#333] border-dashed shrink-0">
           ...
           <div className={`shrink-0 h-8 flex items-center justify-between ${isExpanded ? 'mt-2' : ''}`}>
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-1">
                   <Stripes />
                   <span className="text-xs font-black text-[#444] tracking-widest ml-2">CHRONOS MAG-TAPE</span>
                </div>
                ...
                {isPreview ? (
                  <div className="relative z-50">
                    <button 
                      onClick={onPutBack}
                      className="flex items-center gap-1 bg-retro-light px-3 py-1.5 rounded-sm border-2 border-[#111] shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:translate-y-0.5 hover:shadow-none active:bg-white transition-all text-xs font-bold text-black"
                    >
                      <span>{t.putBack}</span>
                      <ChevronDown size={14} />
                    </button>
