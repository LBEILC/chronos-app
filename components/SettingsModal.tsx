...
import { X, Settings as SettingsIcon, Download, Upload, Volume2, VolumeX, Smartphone, Monitor, Image as ImageIcon, Sparkles, Layout, Maximize, Sun, Globe, Brain, Music, Vibrate, Camera, HardDrive } from 'lucide-react';
...
`}</style>
      ...
      {/* Settings Window */}
      <div className={`relative w-full max-w-md bg-modal-bg border-4 border-modal-border shadow-[8px_8px_0px_rgba(0, 0, 0, 1)] ${isClosing ? 'animate-slide-down-out' : 'animate-slide-up'} overflow-hidden flex flex-col max-h-[70vh] mt-[env(safe-area-inset-top)]`}>

        {/* Header-Shrink 0 to prevent collapsing */}
        <div className="bg-header-bg px-4 py-2 flex justify-between items-center border-b-4 border-modal-border shrink-0 z-10">
          ...
          <button
            onClick={handleClose}
            className="bg-retro-red text-black p-0.5 border border-black hover:bg-red-400 active:scale-90 transition-all"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          <div className="absolute inset-0 bg-modal-bg opacity-90 pointer-events-none" />
            ...
            <div className="space-y-8">
              ...
              <div className="flex items-center justify-between group">
                <div className="flex flex-col">
                  ...
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.language}</div>
                </div>
                <div className="flex bg-terminal-bg p-1 border-2 border-divider rounded-sm shrink-0">
                  <button
                    onClick={() => { setLang('en'); triggerHaptic('light'); }}
                    className={`px-3 py-1 text-xs font-bold transition-all active:scale-95 ${lang === 'en' ? 'bg-retro-amber text-black shadow-[2px_2px_0px_#000]' : 'text-[#666] hover:text-[#888]'} `}
                  >
                    EN
                  </button>
                  <button
                    onClick={() => { setLang('zh'); triggerHaptic('light'); }}
                    className={`px-3 py-1 text-xs font-bold transition-all active:scale-95 ${lang === 'zh' ? 'bg-retro-amber text-black shadow-[2px_2px_0px_#000]' : 'text-[#666] hover:text-[#888]'} `}
                  >
                    中文
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between group">
                <div className="flex flex-col">
                  <div className="flex items-center gap-3 text-retro-light">
                    ...
                    <span className="font-mono text-lg">{t.theme}</span>
                  </div>
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.theme}</div>
                </div>
                <div className="flex bg-terminal-bg p-1 border-2 border-divider rounded-sm shrink-0">
                  <button
                    onClick={() => { setTheme('dark'); triggerHaptic('light'); }}
                    className={`px-3 py-1 text-xs font-bold transition-all active:scale-95 ${theme === 'dark' ? 'bg-yellow-400 text-black shadow-[2px_2px_0px_#000]' : 'text-[#666] hover:text-[#888]'} `}
                  >
                    {t.darkMode}
                  </button>
                  <button
                    onClick={() => { setTheme('light'); triggerHaptic('light'); }}
                    className={`px-3 py-1 text-xs font-bold transition-all active:scale-95 ${theme === 'light' ? 'bg-yellow-400 text-black shadow-[2px_2px_0px_#000]' : 'text-[#666] hover:text-[#888]'} `}
                  >
                    {t.lightMode}
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.smartAnalysis}</div>
                </div>
                <button
                  onClick={() => { setSmartAnalysisMode(!smartAnalysisMode); triggerHaptic('medium'); }}
                  className={`w-16 h-8 border-2 border-divider rounded-full relative transition-all shrink-0 active:scale-95 ${smartAnalysisMode ? 'bg-[#221122]' : 'bg-terminal-bg'} `}
                >
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.crt}</div>
                </div>
                <button
                  onClick={() => { setCrtEnabled(!crtEnabled); triggerHaptic('medium'); }}
                  className={`w-16 h-8 border-2 border-divider rounded-full relative transition-all shrink-0 active:scale-95 ${crtEnabled ? 'bg-[#112211]' : 'bg-terminal-bg'} `}
                >
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.imageStyle}</div>
                </div>
                <button
                  onClick={() => { setImageStylization(!imageStylization); triggerHaptic('medium'); }}
                  className={`w-16 h-8 border-2 border-divider rounded-full relative transition-all shrink-0 active:scale-95 ${imageStylization ? 'bg-[#222222]' : 'bg-terminal-bg'} `}
                >
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    ...
                    <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.sound}</div>
                  </div>
                  <button
                    onClick={() => { setSoundEnabled(!soundEnabled); triggerHaptic('medium'); }}
                    className={`w-16 h-8 border-2 border-divider rounded-full relative transition-all shrink-0 active:scale-95 ${soundEnabled ? 'bg-[#221111]' : 'bg-terminal-bg'} `}
                  >
                ...
                {soundEnabled && (
                  <div className="flex flex-col gap-3 pl-9 animate-fade-in border-l border-divider">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          className="flex-1 h-1 bg-divider appearance-none rounded-full [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-retro-orange [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:shadow-[0_0_5px_rgba(255,85,0,0.5)]"
                        />
                      </div>
                    </div>
...
px-1 py-1.5 text-[9px] font-bold uppercase border transition-all
                                ...
                                : 'border-divider text-subtext hover:border-subtext hover:text-subtext'
                              }
`}
                          >
                            {t.soundTypes[type]}
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.haptic}</div>
                </div>
                <button
                  onClick={() => {
                  ...
                  className={`w-16 h-8 border-2 border-divider rounded-full relative transition-all shrink-0 active:scale-95 ${hapticEnabled ? 'bg-[#221111]' : 'bg-terminal-bg'} `}
                >
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    ...
                    <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.screenshot}</div>
                  </div>
                  <button
                    onClick={() => { setScreenshotMode(!screenshotMode); triggerHaptic('medium'); }}
                    className={`w-16 h-8 border-2 border-divider rounded-full relative transition-all shrink-0 active:scale-95 ${screenshotMode ? 'bg-[#111122]' : 'bg-terminal-bg'} `}
                  >
                ...
                {screenshotMode && (
                  <div className="pt-2 border-t border-divider animate-fade-in flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <select
                        ...
                        className="bg-terminal-bg text-retro-amber border border-divider p-2 font-mono text-xs focus:outline-none focus:border-retro-amber"
                      >
                        {notes.map(n => (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-retro-light">
                        <Layout size={14} className="text-subtext" />
                        <span className="font-mono text-xs text-subtext uppercase">{t.splitTitleMood}</span>
                      </div>
                      <button
                        onClick={() => { setScreenshotSplitView(!screenshotSplitView); triggerHaptic('light'); }}
                        className={`w-10 h-5 border border-divider rounded-full relative transition-colors shrink-0 ${screenshotSplitView ? 'bg-[#222233]' : 'bg-terminal-bg'} `}
                      >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-retro-light">
                        <Layout size={14} className="text-subtext" />
                        <span className="font-mono text-xs text-subtext uppercase">{t.cardMargin}</span>
                      </div>
                      <button
                        onClick={() => { setScreenshotMargin(!screenshotMargin); triggerHaptic('light'); }}
                        className={`w-10 h-5 border border-divider rounded-full relative transition-colors shrink-0 ${screenshotMargin ? 'bg-[#222233]' : 'bg-terminal-bg'} `}
                      >
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-col mb-1">
                  ...
                  <div className="text-[10px] text-subtext font-mono pl-8 pt-1">// {t.settingsTips.data}</div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={handleExport}
                    className="flex items-center justify-center gap-2 px-3 py-2 bg-terminal-bg border border-divider hover:bg-[#222] hover:border-retro-amber hover:text-retro-amber transition-all active:scale-95 text-xs font-bold uppercase tracking-wider"
                  >
                    <Download size={14} />
                    {t.exportData}
                  </button>
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-3 py-2 bg-terminal-bg border border-divider hover:bg-[#222] hover:border-retro-green hover:text-retro-green transition-all active:scale-95 text-xs font-bold uppercase tracking-wider"
                  >
                    <Upload size={14} />
                    {t.importData}
                  </button>
        ...
        <div className="px-4 py-2 bg-terminal-bg text-subtext text-[10px] uppercase font-mono border-t-2 border-modal-border flex justify-between shrink-0 z-10">
