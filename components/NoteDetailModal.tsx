  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 ${isClosing ? 'pointer-events-none' : ''}`}>
      ...
      {selectedImage && (
        ...
        >
          <button 
            className="absolute top-4 right-4 text-white hover:text-retro-red transition-colors flex items-center gap-2 border border-white/20 px-3 py-1 bg-black"
          >
            <X size={20} />
          ...
          <img 
            src={selectedImage} 
            alt="Full view" 
            className={`max-w-full max-h-[90vh] object-contain border-4 border-[#333] shadow-[0_0_50px_rgba(0,0,0,1)] ${imageStylization ? 'grayscale contrast-125' : ''}`}
